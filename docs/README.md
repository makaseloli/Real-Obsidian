...
Fabric `fabric.mod.json` files are generated from shared values in `gradle.properties`, `version.txt`, and the Fabric convention plugin. The convention provides common fields such as the mod id, version, authors, contact URLs, environment, entry points, Java requirement, and Minecraft requirement. Real Obsidian does not apply the Fabric API or config conventions, so generated Fabric metadata does not declare those dependencies.
...
When a version-specific common project such as `1.21.1-common` needs extra entries, append them before a platform binds the declarations:
...
When a platform such as `1.21.1-fabric` or `1.21.1-neo` needs its own entries, append them in the entry point before calling `PlatformConfigRegistrar`:
