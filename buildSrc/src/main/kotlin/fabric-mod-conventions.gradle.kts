...
fun defaultModMetadata(fabricDependencies: Map<String, String>): Map<String, Any?> {
    val entrypoints = linkedMapOf<String, List<String>>()
    if (modFabricEntrypoint.isNotBlank()) {
        entrypoints["main"] = listOf(modFabricEntrypoint)
    }
    if (modFabricClientEntrypoint.isNotBlank()) {
        entrypoints["client"] = listOf(modFabricClientEntrypoint)
    }
